...
# just look at data when beta > 0, beta < 0.1

```{r}
just_beta <- just_beta %>% mutate(seqnames = "chr22") %>%
  mutate(row_num = 1:nrow(.)) %>%
  mutate(beta_low = ifelse(beta > 0 & beta < 0.1, "low", "other"))
just_beta[1:10,]
```
...
just_beta %>% 
  ...
  mutate(count = 1:n()) %>%
  ungroup() %>%
  mutate(length = sequence(rle(.$beta_low)$lengths)) %>%
  group_by(beta_low) %>%
  filter(beta_low == "low", length >= 3) %>% nrow()
```
...
grouped_betas <- just_beta %>% 
  ...
  ungroup() %>% filter(beta_low == "low", length > 2) %>% data.frame() 
grouped_betas[1:10,]
```
