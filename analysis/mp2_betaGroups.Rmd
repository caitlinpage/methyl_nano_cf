...
just_beta %>%
  ggplot(aes(x = beta)) +
  geom_density(linewidth = 3, colour = "purple") +
  theme_bw()
```

```{r}
for_beta %>% distinct(read_id, alpha) %>%
  ggplot(aes(x = alpha)) +
  geom_density(linewidth = 3, colour = "magenta") +
  theme_bw()
```
```{r}
for_beta %>% distinct(read_id, alpha) %>%
  ggplot(aes(x = alpha)) +
  geom_density(linewidth = 3, colour = "magenta") +
  geom_density(data = just_beta, aes(x = beta), linewidth = 3, colour = "purple") +
  theme_bw() +
  labs(x = "Alpha (pink), Beta (Purple)")
```
```{r}
for_beta %>% filter(total >= 3) %>% distinct(read_id, alpha) %>%
  ggplot(aes(x = alpha)) +
  geom_density(linewidth = 3, colour = "magenta") +
  geom_density(data = filter(just_beta, coverage >= 3), aes(x = beta), linewidth = 3, colour = "purple") +
  theme_bw() +
  labs(x = "Alpha (pink), Beta (Purple)")
```
```{r}
...
```{r}
for_beta %>% filter(beta >= 0.4, beta <= 0.6) %>% mutate(medium = "medium") %>%
  ggplot(aes(x = medium, y = alpha)) +
...
```{r}
for_beta %>% filter(total > 2) %>% group_by(start) %>% mutate(mean_alpha = mean(alpha)) %>% distinct(start, beta, mean_alpha, coverage) %>% 
  ggplot(aes(x = beta, y = mean_alpha)) +
  geom_point(aes(size = coverage), alpha = 0.3) +
  geom_abline(colour = "red", linetype = "dashed", linewidth = 1) +
  theme_bw()
```
