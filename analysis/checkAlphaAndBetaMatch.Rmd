...
```
* wgbs_tools (wt) should have a range of unmeth values - file can't be trusted
...
```{r}
indiv_reads %>% group_by(ref_position) %>% .[order(.$ref_position),] %>% mutate(n=n()) %>% filter(n>1) %>% head()
```
```{r}
...
```{r}
overlaps_bed_reads %>% filter(beta_match == TRUE) %>% head()
```
...
```{r}
overlaps_bed_reads %>% filter(call_prob > 0.8, fail == TRUE) %>% group_by(ref_position) %>% mutate(expec_beta_filt = mean(call_code == "m")) %>% ungroup() %>% mutate(match_beta_filt = ifelse(beta == expec_beta_filt, TRUE, FALSE)) %>% group_by(match_beta_filt) %>% filter(match_beta_filt == FALSE) %>% head()
```
...
promoters_txdb <- promoters(TxDb.Hsapiens.UCSC.hg38.knownGene)
```

* promoters should be less methylated than genes(bodies)
* as that is where transcription is activated - and need demethylation to turn on a gene
```{r}
overlaps_bed_reads_filt
```

```{r}
find_overlaps(genes_txdb, as_granges(overlaps_bed_reads_filt)) %>% data.frame() %>% distinct(pos, gene_id, beta) %>%
  ggplot(aes(x = beta)) +
  geom_density() +
  scale_y_continuous(limits = c(0,7)) +
  labs(title = "Distribution of betas overlapping genes")
```

```{r}
find_overlaps(promoters_txdb, as_granges(overlaps_bed_reads_filt)) %>% data.frame() %>% distinct(pos, tx_id, beta) %>%
  ggplot(aes(x = beta)) +
  geom_density() +
  scale_y_continuous(limits = c(0,7)) +
  labs(title = "Distribution of betas overlapping promoters")
```

* yes sense check passes
* plots are on the same scale, and much less full methylation at beta = 1

* note: I only did this with sample hu5.10
** need to repeat for all other samples

##

```{r}
overlaps_bed_reads_filt %>%
  ggplot(aes(x = expec_beta_filt, y = beta)) +
  geom_point()
```
