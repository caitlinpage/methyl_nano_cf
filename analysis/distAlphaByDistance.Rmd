...
```
* still just chrom 22 but still let's make the same plots
```{r}
for_beta
```

```{r}
head(nano_alpha_and_beta)
...
## density on read
```{r}
nano_alpha_and_beta %>% distinct(read_id, total, read_first_cpg, read_last_cpg) %>%
  mutate(width = read_last_cpg - read_first_cpg + 1,
         cpg_density = (total/width)*100) %>%
  ggplot(aes(x = cpg_density)) +
  geom_density()
```
* similar to the rrbs data

## distribution of alpha by distance

```{r}
alpha_distance_nano <- nano_alpha_and_beta %>%
      group_by(read_id) %>%
      mutate(meth_status = call_code,
            dist_to_next = lead(start) - start,
            meth_pattern = paste0(meth_status, lead(meth_status))) %>%
      ungroup() %>%
      fill(dist_to_next, .direction = "down") %>%
      distinct(read_id, meth_pattern, dist_to_next, alpha, total) %>%
      filter(!grepl("A", .$meth_pattern))
nrow(alpha_distance_nano)
alpha_distance_nano[1:10,]
```
...
```{r}
nano_alpha_and_beta <- find_overlaps(as_granges(nano_alpha_and_beta), as_granges(annotations)) %>% data.frame() %>%
...
alpha_distance_nano %>%
  filter(alpha == 0 | alpha == 0.5 | alpha == 1) %>%
  ggplot(aes(x = as.factor(alpha), y = log2(dist_to_next), fill = cgi)) +
  geom_boxplot()
...
alpha_distance_nano %>%
  mutate(alpha_level = case_when(alpha <= 0.2 ~ "Low",
                                 ...
                                 alpha >= 0.8 ~ "High", TRUE ~ "Other"),
         alpha_level = factor(alpha_level, levels = c("Low", "Low-med", "Medium", "Med-high", "High"))) %>%
  ggplot(aes(x = alpha_level, y = log2(dist_to_next), fill = cgi)) +
  geom_boxplot()
```
