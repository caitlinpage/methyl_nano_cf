...
```{r}
fread("/researchers/caitlin.page/rrbs_encode_cell_line/CpG_OB_ENCFF000LYP_bismark_bt2.txt")
```
```{r}
list.files("/researchers/caitlin.page/rrbs_encode_cell_line") %>% .[grep("txt", .)] %>% .[grep("CpG", .)]
```

```{r}
process_wgbs("/researchers/caitlin.page/rrbs_encode_cell_line/CpG_OT_ENCFF000LYO_bismark_bt2.txt")
```
```{r}
path <- "/researchers/caitlin.page/rrbs_encode_cell_line/CpG_OT_ENCFF000LYO_bismark_bt2.txt"
process_wgbs(as.character(path)[1])
```
```{r}
...
```{r}
fread("/researchers/caitlin.page/rrbs_encode_cell_line/CpG_OT_ENCFF000LYO_bismark_bt2.txt")
```


mnmnmmnmjmknkmkhk

* also there was actually a processed file so let's take a squiz at that
```{r}
#read.table("../data/rrbs/pancreatic_cancer/GSM1541788_38-Lg_rep1.cg.txt.gz", header = TRUE)
```
