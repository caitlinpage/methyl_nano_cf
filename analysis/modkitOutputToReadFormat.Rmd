...
```{r}
he_hu5.10 <- he_hu5.10 %>% separate_longer_position(cols = meth_pattern, width = 1) %>% group_by(read_id) %>% mutate(cpg_pos = index_cg:c(index_cg + n() -1)) %>%
...
```{r}
separate_longer_position(he_hu5.10_sample, meth_pattern, width = 1)
```
