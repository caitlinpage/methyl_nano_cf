...
* layers could be optional - maybe they don't want everything
* style of the layers
** maybe they want beta as a line, not as an x


# 1 dot per read (alpha)
* size of dot indicates how many reads have the same alpha/position
* dot is coloured by methylated or not
```{r}
plot_fn <- function(chrom = chrom, region = c(start, end), data_big = data_big, cg_sites = cg_sites) {
  ...
  data2 <- data %>% mutate(alignment_start = start_pos, alignment_end = end_pos)
...
```{r}
plot_fn("chr22", c(10522481, 10524771), alpha_and_beta2_pat, cpg_positions)
```
