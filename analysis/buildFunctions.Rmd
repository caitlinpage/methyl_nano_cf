...
process_wgbs <- function(bismark_modkit, path_to_bed = NULL) {
  #read <- fread(file = path_to_bismark_res)
  read <- bismark_modkit
...
```
* haven't made bed files so will need to make betas myself
```{r}
process_wgbs_short <- function(bismark_modkit, path_to_bed = NULL) {
  #read <- fread(file = path_to_bismark_res)
  read <- bismark_modkit
  names(read) <- c("read_id", "strand", "seqnames", "start", "meth_status")
  read$end <- read$start
