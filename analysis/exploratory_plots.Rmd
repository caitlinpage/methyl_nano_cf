...
```
```{r}
epiread_format %>% filter(num_cg == 1)
```

* add alpha information to epiread format
```{r}
epiread_format <- epiread_format %>%
    mutate(num_meth = stringr::str_count(epiread_format$meth_pattern, "C"), 
         total = stringr::str_length(epiread_format$meth_pattern),
...
# File formats I have
* Epiread
** read based: 1 row per read, with genomic positions of read and read length (though 1 base less than plyranges would want it)
** number of cpgs per read and positions of those cpgs as index of cpgs on chromosome 22 and as genomic positions
** number of methylated and total cpgs in read, as well as alpha value = proportion of methylated cpgs in read
...
* As expected, higher proportion of alpha = 1
* log2 scale doesn't significantly affect the shape of the distribution
...
epiread_format %>%
  ...
  labs(title = "Log2 Distribution of read length in chr22")
```
* There is variability in read length
* Plot looks better in log2 format - max is very far away from mean
* 2nd plot is trimmed to 1kbp: and can see as expected that read length doubles with known histone lengths ~140bp
* Hey this is a type of grouping-esque thing that I could look at in other contexts!!
** wait what is the length supposed to be? - is it more like 175? I can't find where I wrote it down
** or just calculate from the plot to find the peak values
...
```
* so 174 is the value of the first peak
...
epiread_format %>%
  ...
  labs(title = "Log2 Distribution of cpgs in reads in chr22", 
       y = "number of cpgs in read")
```
* Distribution plot is a bit weird the way it jumps up and down
* Mean and median way way smaller than max
* Log2 doesn't really make it look better - kinda makes me think of harmonic distrib (if I have the right one)
...
* cor of 0 means no correlation, so 0.48 from pearson indicates a bit of correlation
* also evident in plot
* but not as strong as expected
* especially dense section in reads < 500bp mostly with not too many cpgs
...
* reads with 1 histone are more than >50% of all reads
* this will probably affect any other analysis I try to do: because groups are very much not evenly sized
...
```{r}
epiread_format %>% filter(histone_groups %in% levels(epiread_format$histone_groups)[1:2]) %>% group_by(histone_groups) %>% mutate(reads_in_group = n()) %>% ungroup() %>% group_by(histone_groups, alpha, reads_in_group) %>% summarise(reads_per_alpha = n()) %>% ungroup() %>% mutate(prop = reads_per_alpha/reads_in_group) 
...
```{r}
epiread_format[7:11,] %>%
  ggplot(aes(x = alignment_start, y = alpha, colour = read_id)) +
  geom_segment(aes(x = alignment_start, xend = alignment_end, y = alpha, yend = alpha))
```
* hey that worked!!
...
```{r}
epiread_format <- epiread_format %>% mutate(gap_to_next_read = lead(alignment_start) - alignment_end)
epiread_format
```
...
```
* ooh that's more fun
* make it as a dot - maybe take the midpoint - then can draw lines between dots to see potential trends



# remove reads with cpg = 1

* I need to filter out the low ones: a read with 1 cpg isn't very useful
