...
```{r}
mantel_haen_dist_next_cpg(filter(for_beta, cgi == "island"), 2)
```
```{r}
try_2 %>%
...
* this is interesting
```{r}
rbind(mutate(try, source = "All_reads"), mutate(try_2, source = "reads_2cpgs")) %>% data.frame() %>% .[order(.$IPD),]
```
...
```{r}
distinct(for_beta_all_mantel, total) %>% .[order(.$total),]
distinct(for_beta_all, total)
```
...
nam <- c()
for(i in 2:4) {
  nam <- c(nam, paste0(i, " cpgs on read"))
}
nam
```
...
plots <- list()
for(i in 2:3) {
  plot <- for_beta_all_mantel %>% 
    filter(total == i) %>%
    ggplot(aes(x = OR, colour = read_id)) +
    geom_density() +
    theme(legend.position = "none") +
    labs(title = paste0(i, " cpgs on read"))
