...
```
```{r}
capture.output(epiDisplay::mhor(mhtable = chisq_table, graph = FALSE)) %>% data.frame() %>% .[4:37,] %>% data.frame()
```
```{r}
capture.output(epiDisplay::mhor(mhtable = chisq_table, graph = FALSE)) %>% data.frame() %>% .[4:39,] %>% data.frame()
```
```{r}
try
```
...
```{r}
mantel_haen_dist_next_cpg <- function(long_data, num_cpg_read) {
...
mantel_haen_dist_next_cpg(for_beta, 5)
```
```{r}
for_beta %>% distinct(total)
```
...
distinct_rows <- for_beta %>%
      ...
      group_by(dist_to_next, meth_pattern) %>%
      summarise(count = n()) %>%
      ungroup() %>% .$dist_to_next %>% 
    unique() %>%
    length()
chi_a <- simplify2array(a)
```
```{r}
b <- for_beta %>%
      filter(total == 5) %>%
...
```{r}
missing_mm <- b %>% group_by(dist_to_next) %>% filter(!any(grepl("MM", meth_pattern))) %>% distinct(dist_to_next) %>% ungroup() %>% .$dist_to_next
length(missing_mm)
for(i in 1:length(missing_mm)) {
  b <- rbind(b, c(missing_mm[i], "MM", 0, "M", "M"))
}

missing_mu <- b %>% group_by(dist_to_next) %>% filter(!any(grepl("MU", meth_pattern))) %>% distinct(dist_to_next) %>% ungroup() %>% .$dist_to_next
length(missing_mu)
for(i in 1:length(missing_mu)) {
  b <- rbind(b, c(missing_mu[i], "MU", 0, "M", "U"))
}

missing_um <- b %>% group_by(dist_to_next) %>% filter(!any(grepl("UM", meth_pattern))) %>% distinct(dist_to_next) %>% ungroup() %>% .$dist_to_next
length(missing_um)
for(i in 1:length(missing_um)) {
  b <- rbind(b, c(missing_um[i], "UM", 0, "U", "M"))
}
missing_uu <- b %>% group_by(dist_to_next) %>% filter(!any(grepl("UU", meth_pattern))) %>% distinct(dist_to_next) %>% ungroup() %>% .$dist_to_next
length(missing_uu)
for(i in 1:length(missing_uu)) {
  b <- rbind(b, c(missing_uu[i], "UU", 0, "U", "U"))
}

b$count <- as.integer(b$count)
b$dist_to_next <- as.integer(b$dist_to_next)
b
b %>% group_by(dist_to_next) %>% mutate(check = n()) %>% ungroup() %>% filter(check < 4)
```

```{r}
b_chi <- b %>% group_by(dist_to_next) %>% 
      reframe(tab = list(xtabs(count ~ cpg_1 + cpg_2, data = cur_data())))
b_chi$tab
```

```{r}
rbind(b, c(26, "MA", 0, "M", "A"))
```

```{r}
try <- capture.output(epiDisplay::mhor(mhtable = chi_a, graph = FALSE))
...
```
```{r}
numeric_lines <- grep("Inf|\\d", try$numbers, value = TRUE)
```
```{r}
try$numbers
```
...
vals <- regmatches(numeric_lines, gregexpr(pattern, numeric_lines))
